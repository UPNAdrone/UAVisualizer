        ...
        while True:
            ...
            if msg is not None:
                ...
                if msg.get_type() == 'SYS_STATUS':
                    ...
                    data_dict = {'timestamp': timestamp, 'data': data.tolist()}  # Convert numpy array to list for serialization
                    ...
                    data_dict = {'timestamp': timestamp, 'data': home_ned.tolist()}  # Convert numpy array to list for serialization
            ...
            if data_dict is not None:
                if len(message_queue) >= MAX_QUEUE_SIZE:
                    ...
                    message_queue.pop(0)
                message_queue.append(data_dict)

            # Print the most recent message and remove it from the queue
            if message_queue:
                await websocket.send(json.dumps(message_queue.pop(0)))  # Serialize the object to JSON before sending
                msg_recv = await websocket.recv()
                recv_timestamp = time.time()
                
                delay = recv_timestamp - json.loads(msg_recv)['timestamp']  # Deserialize received JSON to get timestamp
                print("Delay:", delay)
